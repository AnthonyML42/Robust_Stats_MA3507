...
gross_outlier_sim <- function(N_pts, small_var = 1, big_mean = 0, big_var = 100, contamination_proportion, estimator = "MM", initial_estimator = "S") {
    ...
    x_vals <- seq(1, N_pts)
    small_noise <- rnorm(n = N_pts, mean = 0, sd = sqrt(small_var))
    big_noise <- rnorm(n = n_contaminates, mean = big_mean, sd = sqrt(big_var))
    ...
    y_vals[contamination_idx] <- big_noise
